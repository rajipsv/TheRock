...
- Changed: Increased GHA `miopen` job timeout from 60 to 120 minutes in `fetch_test_configurations.py`; no submodule changes.  
- CI Impact: Apply `test:miopen` and `test_filter:quick` labels to restrict test scope; timeout adjustments override per-test limits from `test_categories.yaml`.  
- Rollout: Canary strategy for gfx family; verify alignment with component-specific test labels and avoid broad test suite execution.